...
describe("TriggerSecondStageUseCase", () => {
    ...
    it("should throw AutomationNotFoundError when automation does not exist", async () => {
        ...
        await expect(
            usecase.execute({ automationId: "unknown" }),
    ...
    it("should throw InvalidAutomationStageError when stage is not TRIAGE", async () => {
        ...
        await expect(
            usecase.execute({ automationId: "a1" }),
        ).rejects.toBeInstanceOf(InvalidAutomationStageError)
    })
    ...
    it("should throw AutomationCanStartDiligenceError when triage is not completed", async () => {
        ...
        await expect(
            usecase.execute({ automationId: "a1" }),
        ).rejects.toBeInstanceOf(AutomationCanStartDiligenceError)
    })
    ...
    it("should create four automations, duplicate documents, emit events, and return IDs", async () => {
        ...
        resultRepository.findLatestWithDocuments.mockResolvedValueOnce({
            result: {
                id: "r1",
                automationId: "auto-triage-1",
                status: "OK",
                onePagerSummary: null,
            },
            outputDocuments: [
                {
                    id: "od1",
                ...
                },
                {
                    ...
                    name: "cs1",
                    status: "OK",
                    sector: "COMPANY_SUMMARY",
                    documentId: "d6",
                    document: {
                        id: "d6",
                        name: "summary.pdf",
                        bucketPath: "gs://b/summary.pdf",
                    },
                },
                {
                    id: "od7",
        ...
        expect(
            docs.some(
                (x: any) => x.automationId === "fi-1" && x.name === "fin.pdf",
            ),
        ).toBe(true)
        expect(
            docs.some(
                (x: any) =>
                    x.automationId === "lg-1" &&
                    (x.name === "legal.pdf" || x.name === "corp.pdf"),
            ),
        ).toBe(true)
        expect(
            docs.some(
                (x: any) =>
                    x.automationId === "cm-1" &&
                    (x.name === "clients.pdf" || x.name === "inv.pdf"),
            ),
        ).toBe(true)
        expect(
            docs.some(
                (x: any) =>
                    x.automationId === "op-1" &&
                    (x.name === "summary.pdf" || x.name === "team.pdf"),
            ),
        ).toBe(true)
        ...
        expect(payloads).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    automationId: "op-1",
                    domain: AgentType.OPERATIONAL,
                }),
                expect.objectContaining({
                    automationId: "cm-1",
                    domain: AgentType.COMMERCIAL,
                }),
                expect.objectContaining({
                    automationId: "fi-1",
                    domain: AgentType.FINANCIAL,
                }),
                expect.objectContaining({
                    automationId: "lg-1",
                    domain: AgentType.CAP_TABLE_AND_LEGAL_REVIEW,
                }),
            ]),
        )
    })
    ...
    it("should rollback and throw DiligenceCreationFailedError when transaction fails", async () => {
        ...
        await expect(
            usecase.execute({ automationId: "auto-triage-1" }),
        ).rejects.toBeInstanceOf(DiligenceCreationFailedError)
    })
})
