...
type StatusLike = 'NOT_STARTED' | 'PENDING' | 'PROCESSING' | 'COMPLETED' | 'FAILED'
...
export class AutomationStatusValidator {
    ...
    static canStartTriage(automation: AutomationLike): boolean {
        return this.isTriage(automation) && this.isStartableStatus(automation.status)
    }
    ...
    static stageFromLabel(label: 'TRIAGE' | 'OPERATIONAL' | 'COMMERCIAL' | 'FINANCIAL' | 'CAP_TABLE_AND_LEGAL_REVIEW'): StageLike {
        ...
        if (label === 'CAP_TABLE_AND_LEGAL_REVIEW') return 'DILLIGENCE_CAP_TABLE_AND_LEGAL_REVIEW'
        return 'TRIAGE'
    }
}