...
export class EnvValidator {
    ...
    private static readonly OPTIONAL_WITH_DEFAULTS: Record<keyof EnvConfig, string> = {
        DATABASE_URL: '',
        REDIS_HOST: '',
        REDIS_PORT: '',
        PORT: '3000',
    ...
    static validateEnvironmentVariables(): EnvConfig {
        ...
        for (const [varName, defaultValue] of Object.entries(this.OPTIONAL_WITH_DEFAULTS)) {
    ...
    private static validateSpecificFormats(config: EnvConfig): void {
        ...
        if (config.REDIS_PORT && isNaN(parseInt(config.REDIS_PORT))) {
            throw new Error(`❌ REDIS_PORT must be a valid number, got: ${config.REDIS_PORT}`)
        }

        // Validate SMTP_PORT is a number
        if (config.SMTP_PORT && isNaN(parseInt(config.SMTP_PORT))) {
            throw new Error(`❌ SMTP_PORT must be a valid number, got: ${config.SMTP_PORT}`)
        }
        ...
        for (const varName of booleanVars) {
            ...
            if (value && !['true', 'false'].includes(value.toLowerCase())) {
                this.logger.warn(`⚠️ ${varName} should be 'true' or 'false', got: ${value}`)
            }
        }

        // Validate URLs
        if (config.AGENT_API_URL && !config.AGENT_API_URL.startsWith('http')) {
            throw new Error(`❌ AGENT_API_URL must be a valid URL, got: ${config.AGENT_API_URL}`)
        }
        ...
        if (config.AGENT_SECRET && config.AGENT_SECRET.length < 32) {
            throw new Error('❌ AGENT_SECRET must be at least 32 characters long')
        }
        if (config.JWT_SECRET && config.AGENT_SECRET && config.JWT_SECRET === config.AGENT_SECRET) {
            throw new Error('❌ JWT_SECRET and AGENT_SECRET must be different values')
        }
    ...
    static getEnvSummary(): void {
        ...
        this.logger.log(`   📊 REDIS: ${process.env.REDIS_HOST || 'localhost'}:${process.env.REDIS_PORT || '6381'}`)
        this.logger.log(`   ☁️ GCLOUD_STORAGE_BUCKET: ${process.env.GCLOUD_STORAGE_BUCKET ? 'configured' : '❌ NOT SET'}`)
        this.logger.log(`   🤖 AGENT_API_URL: ${process.env.AGENT_API_URL ? 'configured' : '❌ NOT SET'}`)
        this.logger.log(`   📄 ONEPAGER_INCREMENTAL: ${process.env.ONEPAGER_INCREMENTAL_ENABLED || 'false'}`)
    }
}
