...
} from "@/shared/errors/queue-error"
...
export class MultiQueueEventBusAdapter implements EventBusPort {
    ...
    constructor(
        ...
        @InjectQueue("chunk-processing-queue")
        private readonly chunkProcessingQueue: Queue,
        @InjectQueue("chunk-retry-queue")
        private readonly chunkRetryQueue: Queue,
    ...
    async emit(eventName: string, data: any): Promise<void> {
        ...
        try {
            const queueName =
                this.eventToQueueMap.get(eventName) || "automation-queue"
