...
export const REDIS_KEYS = {
    AUTOMATION_REPORTS: (automationId: string) => `automation:reports:${automationId}`,
...
export function updateAutomationReportsMetadata(
    ...
    return {
        ...
        metadata: {
            ...
            completedReports: reports.filter(r => r.status === ReportStatus.COMPLETED).length,
            untrackedReports: reports.filter(r => r.status === ReportStatus.UNTRACKED).length,
            failedReports: reports.filter(r => r.status === ReportStatus.FAILED).length,
            lastUpdated: new Date().toISOString(),
        },
    };
}