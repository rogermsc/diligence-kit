...
export class LoginUseCase {
    ...
    async execute(input: LoginInput): Promise<LoginOutput> {
        ...
        if (!user) {
            ...
            this.logger.warn({ event: 'login.failure', reason: 'user_not_found', email, ip, timestamp: new Date().toISOString() });
        ...
        if (!isValidPassword) {
            await this.rateLimiter.recordFailure(email);
            this.logger.warn({ event: 'login.failure', reason: 'invalid_password', email, ip, timestamp: new Date().toISOString() });
            throw new InvalidCredentialsError();
        }

        const refreshTokenId = randomUUID();
        const accessToken = await this.authService.generateToken(user, "1d");
        const refreshToken = await this.authService.generateRefreshToken(refreshTokenId);
        ...
        this.logger.log({ event: 'login.success', userId: user.id, email, ip, timestamp: new Date().toISOString() });
