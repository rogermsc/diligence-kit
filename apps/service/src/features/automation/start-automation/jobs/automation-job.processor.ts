...
import {
    ZipAssembledEventSchema,
    DocumentsUploadedEventSchema,
...
export class AutomationJobProcessor {
    ...
    async handleZipAssembled(job: Job<ZipAssembledEvent>): Promise<void> {
        const {
            ...
            companyId,
            companyName,
            zipFile: zipFileMetadata,
        } = ZipAssembledEventSchema.parse(job.data)
        ...
        try {
            ...
            this.logger.error(
                "Failed to process ZIP assembled and update status for FAILED",
                {
                    automationId,
                    error: error.message,
                },
    ...
    async handleDocumentsUploaded(
        job: Job<DocumentsUploadedEvent>,
    ): Promise<void> {
        const { automationId, companyId, companyName, uploadResult } =
            DocumentsUploadedEventSchema.parse(job.data)
    ...
    async handleAgentNotification(
        job: Job<AgentNotificationEvent>,
    ): Promise<void> {
        const { automationId, companyId, companyName, documents } =
            AgentNotificationEventSchema.parse(job.data)
    ...
    async handleAutomationUpdated(
        job: Job<AutomationUpdatedEvent>,
    ): Promise<void> {
        const { automationId, status } = AutomationUpdatedEventSchema.parse(
            job.data,
        )
