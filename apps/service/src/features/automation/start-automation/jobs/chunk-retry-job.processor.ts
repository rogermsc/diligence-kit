...
import {
    ChunkQueuedForRetryEventSchema,
    ChunkRetryAttemptEventSchema,
...
export class ChunkRetryJobProcessor {
    ...
    async handleChunkQueuedForRetry(
        job: Job<ChunkQueuedForRetryEvent>,
    ): Promise<void> {
        const {
            uploadId,
            chunkNumber,
            ...
            retryAttempt,
            nextRetryAt,
        } = ChunkQueuedForRetryEventSchema.parse(job.data)
        ...
        try {
            ...
            this.logger.error(
                `Failed to process chunk queued for retry:`,
                error,
            )
            throw error
        }
    ...
    async handleChunkRetryAttempt(
        job: Job<ChunkRetryAttemptEvent>,
    ): Promise<void> {
        const {
            uploadId,
            chunkNumber,
            ...
            maxRetries,
            delayMs,
        } = ChunkRetryAttemptEventSchema.parse(job.data)
        ...
        try {
            ...
            this.logger.debug(
                `Chunk ${chunkNumber} re-submitted for processing (attempt ${retryAttempt})`,
            )
        } catch (error) {
            this.logger.error(
                `Retry attempt ${retryAttempt} failed for chunk ${chunkNumber}:`,
                error,
            )
            ...
            if (retryAttempt < maxRetries) {
                ...
                await this.eventBus.emit("chunk.queued.for.retry", {
                    ...
                    nextRetryAt: new Date(
                        Date.now() + this.calculateRetryDelay(retryAttempt + 1),
                    ),
                ...
                this.logger.error(
                    `Chunk ${chunkNumber} failed permanently after ${maxRetries} attempts`,
                )
