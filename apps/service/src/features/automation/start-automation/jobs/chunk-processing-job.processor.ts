...
export class ChunkProcessingJobProcessor {
    ...
    async handleChunkRegistered(job: Job<ChunkRegisteredEvent>): Promise<void> {
        const {
            ...
            companyId,
            automationId,
        } = ChunkRegisteredEventSchema.parse(job.data)
        ...
        try {
            ...
            if (!isPayloadValid) {
                this.logger.warn(
                    "Missing file payload on chunk.registered; scheduling retry",
                ...
                await this.eventBus.emit("chunk.queued.for.retry", {
                    ...
                    nextRetryAt: new Date(
                        Date.now() + this.calculateRetryDelay(1),
                    ),
            ...
            if (isChunkProcessedSuccessfully) {
                ...
                if (isComplete) {
                    const uploadState =
                        await this.chunkRegistry.getUploadState(uploadId)
                    const automationIdFromRegistry =
                        uploadState?.metadata?.automationId || automationId

                    this.logger.log(
                        `All chunks processed for upload ${uploadId}, triggering assembly`,
                        {
                            uploadId,
                            processedCount: result.processedCount,
                            totalChunks: result.totalChunks,
                            ...
                            automationIdInMetadata:
                                uploadState?.metadata?.automationId,
                    ...
                    if (
                        ...
                        automationIdFromRegistry.trim() === ""
                    ) {
                        this.logger.error(
                            `Cannot trigger assembly: automationId is empty for upload ${uploadId}`,
                            {
                                uploadId,
                                automationIdFromRegistry,
                        ...
                        throw new Error(
                            `AutomationId is required for assembly but was empty for upload ${uploadId}`,
                        )
                    }
            ...
            this.logger.error(
                `Failed to process registered chunk ${chunkNumber}:`,
                error,
            )
            throw error
        }
