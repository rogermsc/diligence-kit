...
export class BatchUploadAdapterService implements BatchUploadPort {
    ...
    async uploadChunkBatch(batch: ChunkBatch): Promise<BatchUploadResult> {
        ...
        const uploadTasks = this.createUploadTasks(
            chunks,
            batch.getMetadata().uploadId,
        )
    ...
    calculateOptimalBatchSize(
        ...
        const chunkSizeIsSmall =
            averageChunkSize <= this.CHUNK_SIZE_THRESHOLD_SMALL
        const chunkSizeIsLarge =
            averageChunkSize >= this.CHUNK_SIZE_THRESHOLD_LARGE
    ...
    private createUploadTasks(
        chunks: any[],
        uploadId: string,
    ): ChunkUploadTask[] {
        return chunks.map((chunkItem) => {
            const chunkFile = this.createChunkFile(
                chunkItem.file,
    ...
    private async uploadSingleChunk(task: ChunkUploadTask): Promise<void> {
        try {
            ...
            this.logger.debug(
                `Chunk ${task.chunkNumber} uploaded successfully`,
                {
