...
import { UpdateAutomationStatusInput, UpdateAutomationStatusOutput } from '../data/dtos/update-automation-status.schema';
import { AutomationNotFoundError } from '../domain/errors/automation-errors';

@Injectable()
export class UpdateAutomationStatusUseCase implements Usecase<UpdateAutomationStatusInput, UpdateAutomationStatusOutput> {
    constructor(
        @Inject('AutomationRepository')
        private readonly automationRepository: IAutomationRepository,
    ) { }

    async execute(input: UpdateAutomationStatusInput): Promise<UpdateAutomationStatusOutput> {
