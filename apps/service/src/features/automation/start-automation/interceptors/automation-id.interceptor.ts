...
export class AutomationIdInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        this.logger.log(`INTERCEPTOR AutomationIdInterceptor executado para rota: ${request.path}`, {
        ...
        if (!request.path.includes('/start/') || request.method !== 'POST') {
            this.logger.log(`INTERCEPTOR Ignorando rota: ${request.path} ${request.method}`);
            return next.handle();
        }
        ...
        try {
            ...
            const validatedChunkData = this.chunkValidator.validate(request.body);
            ...
            if (isLastChunk) {
                ...
                this.logger.log(`INTERCEPTOR Generated automation ID for last chunk: ${automationId}`, {
                    chunkNumber: validatedChunkData.chunkNumber,
                    totalChunks: validatedChunkData.totalChunks,
                    identifier: validatedChunkData.identifier
                });
