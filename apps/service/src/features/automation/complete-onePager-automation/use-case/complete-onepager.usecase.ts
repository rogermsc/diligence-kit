...
@Injectable()
export class CompleteOnePagerUseCase implements Usecase<
    CompleteOnePagerInput,
    CompleteOnePagerOutput
> {
    ...
    async execute({
        automationId,
        data,
    }: CompleteOnePagerInput): Promise<CompleteOnePagerOutput> {
        const automation =
        ...
        const isValidAutomationToReceiveOnePager =
            automation.stage === AutomationStageDomain.TRIAGE

        if (!isValidAutomationToReceiveOnePager) {
            throw new AutomationNotCompletedError(
                "Automation must be in TRIAGE stage to receive one pager",
            )
        }

        const companyId =
            await this.automationRepository.getCompanyIdByAutomationId(
                automationId,
            )
            ...
            try {
                const outputDocuments = [
                    ...(data.coverage || []).map((infoType) => ({
                        ...
                        status: "OK" as const,
                        sector:
                            INFO_TYPE_TO_SECTOR[infoType] ||
                            OutputSector.COMPANY_SUMMARY,
                ...
                this.logger.error(
                    `Failed to create Result/OutputDocuments: ${error.message}`,
                )
            }
        }
