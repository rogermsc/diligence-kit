...
export class CompleteOnePagerController {
    ...
    async completeOnePager(@Body() payload: {
        automationId: string;
        onePagerUrl: string;
        ...
        const validatedData = PayloadValidator.validateWithErrorHandling(
            { onePagerUrl: payload.onePagerUrl, coverage: payload.coverage, missing: payload.missing },
        ...
        if (payload.fileIds?.length) {
            try {
                await this.documentRepository.updateOpenaiFileIds(
                    payload.fileIds.map(f => ({ id: f.documentId, openaiFileId: f.openaiFileId }))
                );
                this.logger.log(`Saved ${payload.fileIds.length} OpenAI file IDs for automation ${payload.automationId}`);
            } catch (error) {
                this.logger.error(`Failed to save OpenAI file IDs: ${error.message}`);
            }
        }

        this.logger.log(`✅ OnePager completion via HTTP for automation ${payload.automationId}`);
        return result;
    }

    @Post('complete-onepager-error')
    async completeOnePagerError(@Body() payload: { automationId: string; error: string }) {
