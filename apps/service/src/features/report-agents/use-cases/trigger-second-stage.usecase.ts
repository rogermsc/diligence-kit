...
import {
    ...
    DiligenceAutomationsAlreadyExistError,
...
@Injectable()
export class TriggerSecondStageUseCase implements Usecase<
    TriggerSecondStageInput,
    TriggerSecondStageOutput
> {
    ...
    async execute(
        input: TriggerSecondStageInput,
    ): Promise<TriggerSecondStageOutput> {
        ...
        const completedTriageAutomation =
            await this.automationRepository.findTriageCompletedById(
                input.automationId,
            )
        ...
        const existingDiligenceAutomations =
            await this.automationRepository.findByParentAutomationId(
        ...
        try {
            ...
            const documents = await this.documentRepository.findByAutomationId(
                completedTriageAutomation.id,
            )
            ...
            // Fetch company name
            // Agent webhook: runs as the system, no calling user.
            const company = await this.companyRepository.findByIdAsSystem(
                completedTriageAutomation.companyId,
            )
            ...
            const createdAutomations =
                await this.automationRepository.createMany(automationsToCreate)
            const createdAutomationIds =
                this.mapCreatedAutomationIds(createdAutomations)

            // Build payload with documents and company name
            const automations: StartReportsPayload[] =
                SectorDomainMapperHelper.getAllDiligenceDomains().map(
                    (domain) => ({
            ...
            return {
                ...
                capTableAndLegalReviewId:
                    createdAutomationIds[AgentType.CAP_TABLE_AND_LEGAL_REVIEW],
            }
        } catch (error) {
            if (
            ...
            this.logger.error(
                "Failed to create diligence automations",
                error.stack,
            )
    ...
    private buildAutomationsData(input: {
        parentAutomationId: string
        companyId: string
    }): CreateDiligenceAutomationData[] {
        return SectorDomainMapperHelper.getAllDiligenceDomains().map(
            (domain) => ({
    ...
    private mapCreatedAutomationIds(
        createdAutomations: any[],
