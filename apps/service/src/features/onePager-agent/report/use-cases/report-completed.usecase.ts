...
@Injectable()
export class ReportCompletedUseCase implements Usecase<
    ReportCompletedInput,
    void
> {
    ...
    async execute(input: ReportCompletedInput): Promise<void> {
        try {
            ...
            this.logger.log(
                `Processing report completion for automation: ${automationId}`,
            )
            ...
            const completedReports = reports.filter((report) =>
                ReportHelper.isCompleted(report),
            )

            this.logger.log(
                `All reports completed for automation ${automationId}`,
                {
            ...
            this.logger.error(
                `Error processing report completion for automation ${input.automationId}: ${error.message}`,
            )
            throw error
        }
    }
