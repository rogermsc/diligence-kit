...
export class DelegateSpecificProcessReportUseCase {
    private readonly logger = new Logger(DelegateSpecificProcessReportUseCase.name);
    ...
    async execute(payload: ReportPayload): Promise<void> {
        ...
        try {
            const reportAutomation = await this.automationRepository.findById(payload.automationId);

            if (!reportAutomation) {
                this.logger.error(`❌ Automation not found: ${payload.automationId}`);
                throw new AutomationNotFoundError();
            }

            const expectedStage = SectorDomainMapperHelper.mapDomainToStage(payload.domain);

            const isAgentTypeValidByStage = reportAutomation.stage === expectedStage;

            if (!isAgentTypeValidByStage) {
                this.logger.error(`❌ Agent type does not match automation stage`, {
                    automationId: payload.automationId,
                    automationStage: reportAutomation.stage,
                    agentDomain: payload.domain,
                    expectedStage
                });
                throw new InvalidAutomationDomainError(payload.automationId, payload.domain);
            }
